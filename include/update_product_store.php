...
$mode = $_POST['mode'];
if($mode == 'get'){
    $get = $pdo->prepare("  SELECT SA.* , AC.Cover_Type
                            FROM `store_album` SA
    ...
    $stm = $pdo->prepare("".$sqlMode." `store_album` SET `item_name`=?, `id_Album_Cover`=?, `soft_cover`=?, 
                        `hard_cover`=?,`amount`=?,`price`=? ".$file.$where."");
    $stm->bindValue(1, $_POST['itemName']);
    $stm->bindValue(2, $_POST['coverType']);
    $stm->bindValue(3, $softCover);
    $stm->bindValue(4, $hardCover);
    $stm->bindValue(5, $_POST['amount']);
    $stm->bindValue(6, $_POST['price']);
    $stm->execute();

    header("Location: /aroma?page=store-list");
}
